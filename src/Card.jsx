const Cards = (props)=>{
  ...
  <a className="relative mx-2 mt-3 flex h-60 overflow-hidden rounded-xl" href="#">
    <img className="object-cover h-auto  max-w-full" src={props.image} alt="product image" />
    <span className="absolute top-0 left-0 m-2 rounded-full bg-black px-2 text-center text-sm font-medium text-white">{props.discount}% OFF</span>
  </a>
  <div className="mt-4 border-t rounded-lg border-slate-300 m-2 p-2 px-5 pb-5">
    <a href="#">
      <h5 className="text-xl tracking-tight font-semibold text-slate-900">{props.title}</h5>
    </a>
    <div className="mt-2 mb-5 flex items-center justify-between">
      <p>
        <span className="text-3xl font-bold text-slate-900">${props.price}</span>
        <span className="text-sm text-slate-900 line-through">{props.discPrice}</span>
      </p>
